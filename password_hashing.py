...
def derive_key(hash_length: int, password: bytes, salt_length: int, mode: int) -> str:
    ...
    if mode == 0:
        ...
        ph = PasswordHasher(time_cost=argon_time_cost[0], memory_cost=argon_memory_cost[0], parallelism=argon_parallelism[0], hash_len=hash_length, salt_len=salt_length, encoding=argon_encoding)
        hash = ph.hash(password=password, salt=argon_salt)

    # Normal Mode
    elif mode == 1:
        argon_salt = secrets.token_bytes(salt_length)
        ph = PasswordHasher(time_cost=argon_time_cost[1], memory_cost=argon_memory_cost[1], parallelism=argon_parallelism[1], hash_len=hash_length, salt_len=salt_length, encoding=argon_encoding)
        hash = ph.hash(password=password, salt=argon_salt)

    # Overkill Mode
    elif mode == 2:
        argon_salt = secrets.token_bytes(salt_length)
        ph = PasswordHasher(time_cost=argon_time_cost[2], memory_cost=argon_memory_cost[2], parallelism=argon_parallelism[2], hash_len=hash_length, salt_len=salt_length, encoding=argon_encoding)
        hash = ph.hash(password=password, salt=argon_salt)

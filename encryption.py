...
def xchacha_decrypt(
    xchacha_key: bytes, xchacha_nonce: bytes, ciphertext: bytes
) -> bytes:
    """
...
def xchacha_camellia_aes_encrypt(
    xchacha_key: bytes, camellia_key: bytes, aes_key: bytes, plaintext: bytes
    ...
    camellia_cipher = Cipher(
        algorithm=algorithms.Camellia(key=camellia_key),
        mode=modes.OFB(camellia_nonce),
        backend=default_backend,
    )
    camellia_encryptor = camellia_cipher.encryptor()
    camellia_ciphertext = (
        camellia_encryptor.update(xchacha_ciphertext) + camellia_encryptor.finalize()
    )

    # AES-256-CTR
    aes_nonce = secrets.token_bytes(16)
    aes_cipher = Cipher(
        algorithm=algorithms.AES256(key=aes_key),
        mode=modes.CTR(aes_nonce),
        backend=default_backend,
    )
    aes_encryptor = aes_cipher.encryptor()
    aes_ciphertext = (
        aes_encryptor.update(camellia_ciphertext) + aes_encryptor.finalize()
    )

    return xchacha_nonce, camellia_nonce, aes_nonce, aes_ciphertext


def xchacha_camellia_aes_decrypt(
    xchacha_key: bytes,
    xchacha_nonce: bytes,
    camellia_key: bytes,
    camellia_nonce: bytes,
    aes_key: bytes,
    aes_nonce: bytes,
    ciphertext: bytes,
) -> bytes:
    ...
    xchacha_ciphertext = (
        camellia_decryptor.update(camellia_ciphertext) + camellia_decryptor.finalize()
    )
